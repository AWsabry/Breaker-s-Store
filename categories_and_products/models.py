...
class Game(models.Model):
    ...
    background_image = models.ImageField(
        upload_to="games", blank=True, validators=[_ext_photo])
    description = models.TextField(blank=True)
    instruction = models.TextField(
        blank=True, help_text='Write a guide for installing the codes in the game platfrom', default=' ')
...
class Code_Categories(models.Model):
    ...
    image = models.ImageField(
        upload_to="codeCategories", blank=True, validators=[_ext_photo])
    background_image = models.ImageField(
        upload_to="codeCategories", blank=True, validators=[_ext_photo])
    active = models.BooleanField(default=True)
    created = models.DateTimeField(auto_now_add=True)
    game = models.ForeignKey(
        Game, on_delete=models.CASCADE, blank=True, null=True,)
    price = models.FloatField(default=0)
    Most_Popular = models.BooleanField(default=False)
    Best_Offer = models.BooleanField(
        default=False, verbose_name="Best Products")
